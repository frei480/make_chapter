...
RULES = """
Проанализируй фрагмент субтитров видео.
...
- не пересказывай текст дословно
- используй таймкод начала блока
...
async def summarize_chunk(chunk: list[str], i: int) -> str:
    ...
    response = await client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Ты создаешь оглавление для видео. Отвечай только на русском языке"
                + RULES,
            },
            {"role": "user", "content": "\n".join(chunk)},
        ],
